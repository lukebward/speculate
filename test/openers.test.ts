...
describe('learner openers', () => {
  ...
  it('loads legacy opener evidence while discarding duplicate latency estimates', () => {
    const l = learnerAt();
    l.importState({
      transitions: [],
      openers: [{ server: 'gh', tool: 'list_issues', argsRepr: '{}', count: 2, latencyMs: 8.8 }],
    });
    expect(l.openerPredictions('gh')).toEqual([
      expect.objectContaining({ tool: 'list_issues', args: {}, horizon: 'standing' }),
    ]);
    expect(l.openerPredictions('gh')[0]).not.toHaveProperty('expectedLatencyMs');
    expect(l.exportState().openers![0]).not.toHaveProperty('latencyMs');
  });
