...
The current repeated-workflow benchmark also completed against a fixed `d2445a5` source/build snapshot: `npm run bench -- --latency 400`, 240 replay records, with 60 held-out sessions per arm. Across 336 held-out requested calls, current Speculate had 146 joins, 0 ready hits, 228 speculative calls and 82 settled unused calls. Aggregate measured tool wait was 129.67 s versus 137.87 s with speculation off; mean per-call wait was 385.92 ms versus 410.33 ms. These are existing MCP-only synthetic results, not native-client or new-observer performance. The [baseline summary](observer-baseline-results.json) records all holdout aggregates.

A [native tool-shape probe](observer-native-tool-shapes.md) verified Claude's flat MCP name/schema layout. Installed Codex0.154.0 instead exposed deferred tools behind the `functions.exec` custom-tool namespace, with no MCP alias in initial model requests. Flat Codex function fixtures describe a supported API variant, not verified native-default stream coverage. Opaque code calls must remain unchanged and ineligible for stream prefetch. Codex also exited zero without a model response in the fake-provider probe, so final production smoke tests must verify model completion, not just exit status.
