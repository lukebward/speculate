...
A per-server transition model over observed calls, learned live from the session. The durable bigram prior backs off from a bounded three-call context (preceding tool, trigger argument shape, and result shape) once that context has enough evidence, so repeated branches can rank differently without sparse histories erasing the base rate. Argument templates hold competing sources (trigger args, bounded paths into parsed results, common deterministic string/number transforms, and constants) and score them against traffic. Tool-name morphology plus target input schemas can also materialize conservative list→detail predictions before the first observed transition; ambiguous or partial schemas abstain. One unexplainable value cannot disable a transition forever, and predictions remain all-or-nothing. Ranking/eviction and persisted feedback decay by elapsed time; alternative argument sets have separate feedback identities. Persisted state is workspace/upstream/account scoped, tool results remain memory-only, and concurrent writers merge learned entities and feedback deltas.
