...
impl Provider {
    pub fn new(ctx: &pulumi::Context, name: &str, args: ProviderArgs, options: pulumi::ResourceOptions) -> Provider {
        let resource = ctx.register_resource(pulumi::RegisterRequest {
            type_: "pulumi:providers:ref-ref".to_string(),
            name: name.to_string(),
            custom: true,
            remote: false,
...
impl Resource {
    pub fn new(ctx: &pulumi::Context, name: &str, args: ResourceArgs, options: pulumi::ResourceOptions) -> Resource {
        let resource = ctx.register_resource(pulumi::RegisterRequest {
            type_: "ref-ref:index:Resource".to_string(),
            name: name.to_string(),
            custom: true,
            remote: false,
            version: "12.0.0".to_string(),
...
pub mod types {
    ...
    impl DataArgs {
        pub fn into_inputs(self) -> std::vec::Vec<(std::string::String, pulumi::Output<pulumi::PropertyValue>)> {
            ...
            if let Some(v) = self.inner_data_list {
                inputs.push(("innerDataList".to_string(), pulumi::output::all(v.into_iter().map(|e| e.into_output()).collect()).cast()));
            }
            {
                let v = self.integer;
                inputs.push(("integer".to_string(), v.cast()));
            }
            if let Some(v) = self.optional_inner {
                inputs.push(("optionalInner".to_string(), v.into_output()));
            }
            {
                let v = self.string;
    ...
    impl pulumi::FromPropertyValue for Data {
        fn from_property_value(v: pulumi::PropertyValue) -> pulumi::Result<Self> {
            ...
            Ok(Data {
                ...
                inner_data_list: pulumi::convert::from_property_map(&m, "innerDataList")?,
                integer: pulumi::convert::from_property_map(&m, "integer")?,
                optional_inner: pulumi::convert::from_property_map(&m, "optionalInner")?,
