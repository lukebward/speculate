...
**What the cap cuts on: expected time saved, not probability.** `confidence × rule effectiveness` answers "how likely is this?", which is only half of "is this worth a slot?". A 50 ms call at 0.8 used to outrank a 2 s call at 0.3 though the second is worth roughly ten times more of the agent's waiting, and the cap threw away the wrong one. Following PASTE's utility score (Appendix A — it ranks by `p·T` over cost and launches greedily while slack remains), the per-trigger cap now cuts on `score × expected upstream ms`. The latency term is a **decayed mean per (server, tool)**, measured from calls Speculate already makes — every speculative call the executor issues, hit or wasted, plus every real call the proxy forwards — using the same `decayedScore` helper as §13.16, with a five-minute time constant rather than the learner's fourteen days, because "how long is this server taking right now" is a question about the current network path, not about whose workflow this is. It is session-scoped and never persisted; nothing new reaches the state file. A tool with no measurement falls back to its server's mean, and a server with no measurements at all falls back to the previous ordering **exactly** — a prediction batch is single-server by construction, so it is never half-priced.

**Value decides membership; probability decides order.** The kept batch is then re-sorted by score, i.e. the old ordering, and the executor's drain queue is deliberately left on confidence. The two decisions ration different resources. The cap and the per-minute budget ration *launches*, where a slot spent on a cheap call is a slot spent, so `p·T` is right. A busy slot rations *upstream time* — the stdio idle-only rule (§3.1) runs one speculative call at a time and a real call arriving mid-flight is credited the work already done — and under a time budget the greedy value-per-unit-cost ratio is `(p·T)/T = p`. Firing the expensive bet first there buys nothing and can starve a likelier cheap one: with A(p=.9, T=100 ms) and B(p=.1, T=1000 ms) in a 500 ms window, confidence order leaves an expected 60 ms of wait and value order 140 ms. Re-sorting the survivors is therefore a no-op on a parallel upstream and weakly better on a serial one.

**This is invisible to the offline eval, by construction.** `npm run eval` scores rank-of-actual-call and is latency-blind: the same ranking change that is worth wall clock cannot move recall by a single pair, and it does not (0.8463 before and after). The bundled `npm run bench` cannot see it either at default settings — every tool in the mock has the *same* injected latency, so there is nothing to prioritize, and even with per-tool latencies (`--hetero`) the profile never offers more candidates than the cap holds, so the cut never fires. Measured in the regime where it does fire (`--hetero --cap 1`): 4.50 s → 4.05 s of tool wait, a −27% → −34% cut. That is a constructed workload, and it is named as one.
...
- **Long-horizon predictions expire sooner.** A prediction whose arguments all derive from the call that just completed claims "this is the next call"; one carrying a memorized literal, or a session opener with no trigger at all (§13.15), claims only "you will ask for this at some point". The second kind waits longer in the buffer, so it is fetched at a **fraction of the resolved TTL** (`LONG_HORIZON_TTL_FACTOR`, default 0.5; operator-overridable per server as `speculation.longHorizonTtlFactor`). The multiplier applies to whatever the normal resolution order produced, so an operator's per-tool freshness decision still sets the ceiling, and a per-tool TTL of 0 stays disabled rather than being revived.
...
|---|---|---|---|---|---|---|---|
| all | 1007 | 1460 ms | 1460 ms | 2960 ms | 0.000 | 1.008 | 1794 |
| next-call | 764 | 1460 ms | 1460 ms | 2960 ms | 0.000 | 1.010 | 1338 |
| standing (memorized) | 243 | 1460 ms | 1460 ms | 1460 ms | 0.000 | 1.000 | 456 |

**The answer to the question the instrument was built to ask is: no, better prediction did not move entries toward expiry.** Against a deliberately narrower model (`maxPredictionsPerTrigger: 1`, roughly the pre-§13.18 one-candidate world) recall@3 is 0.580 vs 0.836 — and the age distribution is *identical*: p50/p95/max unchanged, mean lead 1.011 → 1.008, i.e. very slightly **fresher**. The extra hits §13.18 bought are all claimed by the very next call (999 of 1007 at lead 1, 8 at lead 2), so they arrive at the same 1460 ms as everything else. The corpus's fixed 1.5 s spacing means the ages are close to a point mass; the sensitive statistic here is the **lead**, and the suite pins that the ages do move when consumption is delayed (triple the spacing, triple the age) or the TTL is shrunk.

**Long-horizon predictions now expire sooner** (§6.2): `Prediction.horizon` is `'standing'` when at least one argument came from a `const` source — a remembered literal rather than something read off the call that just happened — or when the prediction is a §13.15 session opener, which has no trigger at all. Standing bets are fetched at `LONG_HORIZON_TTL_FACTOR` (0.5) of the resolved TTL. **Honest limits on that, both ways.** The corpus does not support the const half empirically: standing predictions are consumed at lead exactly 1.000 in every archetype, the same age as derived ones, so on this corpus the shortening is a bound on the worst case rather than a measured improvement — and it is measurably free, since 15 s is still ten times the observed maximum age (turning the multiplier off changes nothing: 1007 hits either way). The opener half, which is where the horizon argument actually bites, is **not measurable here at all**: the corpus has no proxy-start-to-first-call gap to fire openers into, and inventing one would just return the number chosen. Openers are the class most likely to be served at 25–30 s of age, and they are also the class that a halved TTL will cost the most hits; `ageAtHit` in production is the instrument that settles it, and the trade is deliberate — prefer a miss to a stale serve.
