...
- **Per-tool TTL** from the server profile, defaulting to **30 s**, capped at a few minutes. The prefetch-to-use gap is typically seconds, so short TTLs retain most of the win while bounding staleness. The MCP spec has [draft caching metadata](https://modelcontextprotocol.io/specification/draft/server/utilities/caching) (`ttlMs`, `cacheScope`) — when servers ship it, server-declared TTLs override profile defaults (bounded by the operator cap), which both improves freshness handling and future-proofs the design.
